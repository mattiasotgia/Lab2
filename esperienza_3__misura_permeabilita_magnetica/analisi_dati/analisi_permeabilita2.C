...
// result 
void analisi_RLC_filter(std::string file, double* params, TCanvas* canvas, int position, double fitmin = -1, double fitmax = -1, std::string fitoption = "");
...
void analisi_permeabilita2(){
    ...
    // result libero = 
    analisi_RLC_filter("presa_dati_libero.txt", lib0, c1, 0, 8e2, 3.5e4);
    // result m1 = 
    analisi_RLC_filter("presa_dati_materiale1.txt", mat1, c1, 2, 7e2, 7e3);
    // result m2 = 
    analisi_RLC_filter("presa_dati_materiale2.txt", mat2, c1, 4);
...
void analisi_RLC_filter(std::string file, double* params, TCanvas* canvas, int position, double fitmin = -1, double fitmax = -1, std::string fitoption = ""){
    ...
    TF1* H_fit = new TF1("H_f", "1/sqrt(pow(1+[1]/[0], 2)+pow(2*[4]*x*[2]-1/(2*[4]*x*[3]), 2)/([0]*[0]))");//"1/sqrt([0]+[1]*(pow(x/[2]-[2]/x, 2)))"); // ! Controllare formule
    ...
    TF1* phi_fit = new TF1("phi_f", "-atan((2*[4]*[2])-1/(2*[4]*x*[3])/([0]+[1]))");//"-atan([1]*(x/[2]-[2]/x)/[0])"); // ! Controllare formule
    phi_fit->SetParameters(params[0], params[1], params[2], params[3]);
    phi_fit->FixParameter(4, M_PI);

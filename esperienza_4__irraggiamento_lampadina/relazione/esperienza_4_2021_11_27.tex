...
\begin{figure*}
    ...
    \caption{\textbf{Applicazione del secondo modello di analisi dei dati}. A sinistra abbiamo i diversi dati raccolti su cui eseguiamo un fit lineare entro un intorno del punto che vogliamo misurare, per poter effettuare una misura ad una data distanza temporale, poiché i dati non si presentano continui. Si può inoltre osservare come di fatto dopo circa \SI[parse-numbers=false]{30/40}{\second} il comportamento della tensione diventi lineare, in accordo con quanto ipotizzato possa essere il suo comportamento pratico. A destra invece troviamo più chiaro il comportamento $d^{a}$ come  previsto, anche se qualitativamente non è facile individuare il range in cui $a$ può trovarsi. È evidenziato in rosso la sovrapposizione delle bande di errore dopo una successiva analisi, e si può osservare che nel primo punto queste sono discretamente differenti, come appunto imposto a seguito delle considerazioni effettuate. }
    \label{fig:plot:mod2}
\end{figure*}

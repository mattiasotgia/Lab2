...
ed effettuando 5 diverse misure del valore di $V_{out}$ per 5 diversi valori di $V_{in}$ e facendo quindi un fit secondo l'equazione $V_{out}=G_{\text{CM}}\cdot V_{in}+q$ scegliendo come parametri $G_{\text{CM}}$ e $q$ (I valori raccolti si trovano in \reftab{??}). Troviamo quindi che i valori dei parametri sono rispettivamente
\begin{align*}
...
Misurato il $G_{\text{CM}}$, procediamo a calcolare il valore di $G_{\text{diff}}$ usando lo stesso metodo con cui abbiamo trovato $G_{\text{CM}}$ senza però impostare $V_{\text{non-inv}}=V_{\text{inv}}$ (I valori raccolti si trovano in \reftab{??}). I valori dei parametri che il fit ci restituisce sono
\begin{align*}
...
\section*{Presa dati}
Attraverso una interfaccia seriale e un programma di interpretazione del seriale possiamo acquisire a intervalli temporali regolari il valore di tensione misurato dal multimetro da banco Keitley.
...
\begin{figure*}[p]
    \raggedright
    \includegraphics[width=0.8\textwidth]{Guadagno__invertente.pdf}
    \includegraphics[width=0.8\textwidth]{Guadagno__noninvertente.pdf}
    \caption{}
    \label{fig:guadagno_inv_noninv}
\end{figure*}
\begin{figure*}
    \raggedright
    \includegraphics[width=0.8\textwidth]{GuadagnoMC.pdf}
    \includegraphics[width=0.8\textwidth]{GuadagnoDIFF.pdf}
    \caption{}

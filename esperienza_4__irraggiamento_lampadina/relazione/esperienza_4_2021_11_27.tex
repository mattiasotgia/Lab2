...
\begin{figure}
    \begin{circuitikz}
        ...
        \draw 
        (0,0) node[op amp, noinv input up, scale=0.5, anchor=-](OA1){\texttt{OA1}}
        (OA1.+) -- ++(-1, 0) node[above]{{\tiny \texttt{Non inverting input}}}
        (0,-3) node[op amp, scale=0.5, anchor=-](OA2){\texttt{OA2}}
        ...
        (3, -2) node[op amp, noinv input up, scale=0.5, anchor=-](OAout){\texttt{OA out}}
...
Misurato il $G_{CM}$, procediamo a calcolare il valore di $G_{diff}$ usando lo stesso metodo con cui abbiamo trovato $G_{CM}$ senza però impostare $V_{inv}=V_{non-inv}$ (I valori raccolti si trovano in \reftab{??}). I valori dei parametri che il fit ci restituisce sono
\begin{align*}
    G_{diff} &=\num{183.238+-15.5928} \\
    q &=\SI{-0.044431+-0.449922}{\volt} \\
\end{align*}
La quota è compatibile con zero e il valore di $G_{diff}$ è compatibile con il guadagno teorico che avevamo calcolato quando abbiamo scelto le resistenze da inserire nel circuito ($G_{teorico}=$\num{1.68 +- 0.12e2}). Inoltre vediamo anche che $G_{diff}$ è diversi ordini di grandezza più grande di $G_{CM}$ per cui possiamo considerare quest'ultimo trascurabile. Infatti il rapporto di reiezione di modo comune (CMRR) è pari a \num{13.777+-1.6063e3}.

Un altra caratteristica che possiamo misurare è la banda passante dell'amplificatore. Per fare ciò impostiamo la tensione di ingresso a diverse frequenze e raccogliamo i valori di $V_{out}$ per costruire un diagramma di Bode della funzione di traferimento e calcolare la frequenza di taglio 
